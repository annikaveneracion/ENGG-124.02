...
int xVal, yVal, ldr;      // Data received from the serial port
int yValLow, yValHigh, ldrLow, ldrHigh;
int[] yValues, ldrValues;
PrintWriter output;
...
int getY3(int ldr) {
  return (int)(ldr / 1023.0f * height) -1 ;
}
...
void draw()
{
  ...
  drawGrid();
    for (int x=1; x<width; x++) {
      ...
      stroke(0,0,255);
      line(width-x,   height-1-getY3(ldrValues[x-1]), 
           width-1-x, height-1-getY3(ldrValues[x]));
    }
    output.println((millis()/1000) + "s (" + millis() + "ms): " + yVal + ", " + ldr);
    println((millis()/1000) + "s (" + millis() + "ms): " + yVal + ", " + ldr);
